...
BASE_URL = "https://api.intelipost.com.br/api/v1/shipment_order/{}"
...
for pedido_numero in pedidos:
    print(f"\n🔎 Buscando pedido: {pedido_numero}")

    url = BASE_URL.format(pedido_numero)
    response = requests.get(url, headers=headers)

    if response.status_code != 200:
        print(f"❌ Erro {response.status_code} no pedido {pedido_numero}")
        continue
